/*
 * 	캡슐화 (은닉)
 * 	 접근 제한자를 이용하여 접근 제한 하는 경우로 
 * 	  - 외부에서 변수에 직접 접근 x
 * 	  - 직접 접근 안되니 메서드를 통해서 변수의 값을 변경 및 참고
 *  
 *    왜? 이런 방식을 사용할까?
 *    - 잘못된 값(데이터)이 변수의 값이 되는 것을 방치하기 위해서...
 *    - 접근 권한이 없는 사룔자에 의한 데이터 접근을 방지하기 위해서...
 *    
 *    어떻게?
 *    변수 선언 앞에 private
...
//		myBirthDate_goodCase.day = 32;	//접근 제한자 private으로
		myBirthDate_goodCase.setDay(32);
		System.out.println("My birth day is " + myBirthDate_goodCase.getDay());
	}
