...
public class Pen {
	// 클래스 안에 코드 
	// attribute(속성, 멤버변수), constructor(생성자), method(메서드)
...
//	}
	 ...
	 * 	- 기본 생성자는 매개변수를 가지고 있지 않은 형태로, 자동으로 만들어지는 경우에느 내용이 없음
	 * 
	 * 	## 생성자 만드는 규칙
	 * 	1. 생성자 이름은 클래스 이름과 대소문자까지 정확히 같아야 합니다.ㅣ
	 * 		클래스 이름과 다른 이름으로 만들면 그것은 메서드 취급!
	 * 	2. 생성자는 메서드와 달리 반황 유형을 저지 않습니다.
...
//	public Pen(String color, int length) {		// 오류
//		this.color = color;
//		this.length = length;
//		this.price = price;
//	}
