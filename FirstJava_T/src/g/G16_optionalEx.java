...
public class G16_optionalEx {
	...
	/*
	 * 	집계 메서드
	 * 	 스트림 API에서 요소들의 최소값, 최대값, 합계, 평균값 갯수 등을 구할 수 있는 메서드를 제공
	...
	public static void main(String[] args) {
		...
		System.out.println("요소들 중 첫번쨰 값 : " + first.getAsInt());
		...
		// Optional XXX , isPresent() - 값이 있으면 true, isEmpty() - 값이 없으면 true
		...
		System.out.println("요소들 중 최대값 : " + min2);
		
		// 요소가 존재하면 실행
		list2.stream().mapToInt(Integer::intValue).findFirst().ifPresent(a -> System.out.println("요소의 첫번재 값 : " + a));
		
		// 	사용자 집계 메서드 - reduce() 메서드 
		// 	사용자가 다양한 집계 결과물을 생성할 수 있는 메서드
		// 	reduce 메서드는 매개변수 타입이 XXXOperator를 이용하고,
		...
		areaSum = list.stream().mapToDouble(Shape1::area).reduce((a, b) -> a + b).getAsDouble();
		System.out.println("reduce(Operator)를 이용한 면적 합계 : " + areaSum);
		
		areaSum = list.stream().mapToDouble(Shape1::area).reduce(0,  (a,b) -> a + b);
