...
public class G12_mappingEx {
	...
	/*
	 * 	매핑 (Mapping) : 컬렉션 플레임워크에 Map에 key와 value과 같이 묶어주는 것과 같은
	 * 					형태로 만들어 주는 개념. 매핑은 스트림의 데이터를 매개변수로 받아서
	 * 					가공처리 후 새로운 스트림을 만들어 리턴(데이터 변환을 위한 기능)
	 * 
	 * 	메서드 : flatMapXXX(), mapXXX(), addDoubleStream(), asLongStream, boxed()
	 */

	public static void main(String[] args) {
		// flatMapXXX() 메서드 ㅣ 다른 타입의 요소를 가진 스트림을 반환.
		// 리턴 타입 : Stream<R>, DoubleStream, IntStream, LongStream
		
		// 문자열을 공백으로 분리해서 매핑
		List<String> list1 = Arrays.asList("동해물과", "백두산이", "마르고 닳도록");
		list1.stream().flatMap(data -> Arrays.stream(data.split(" "))).forEach(word -> System.out.print(word));
		System.out.println();
		...
		list4.stream().mapToInt(s -> s.length()).forEach(len -> System.out.print(len + " "));
		System.out.println();
